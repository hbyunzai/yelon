...
buildCLI() {
  ...
  if [[ ${COPY} == true ]]; then
    if [[ ${CLONE} == true ]]; then
      cloneScaffold
      echo ">>> copy yelon/ng-yunzai files via travis mode"
      copyFiles 'ng-yunzai/' ${DIST}
    else
      echo ">>> copy work/ng-yunzai files via dev mode"
      copyFiles "${PWD}/ng-yunzai/" ${DIST}
    fi
  else
...
integrationCli() {
  ...
  INTEGRATION_SOURCE=${PWD}/integration
  cd ${INTEGRATION_SOURCE}
  echo ">>> Copy ng-yunzai, Current dir: ${PWD}"
  rsync -a ${DIST} ${INTEGRATION_SOURCE}/node_modules/ng-yunzai
  echo ">>> Running ng g ng-yunzai:ng-add"
  ng g ng-yunzai:ng-add --defaultLanguage=en --codeStyle=true --form=true --mock=true --i18n=true
  echo ">>> Install dependencies"
  npm i
  echo ">>> Copy again ng-yunzai"
  rsync -a ${DIST} ${INTEGRATION_SOURCE}/node_modules/ng-yunzai
  echo ">>> Copy @yelon/*"
  echo ">>>>>> Clone yelon & cli dist..."
  git clone --depth 1 https://github.com/hbyunzai/yelon-builds.git
  rsync -a ${INTEGRATION_SOURCE}/yelon-builds/ ${INTEGRATION_SOURCE}/node_modules/
