...
import { addImportToModule, addValueToVariable, getProject, importInStandalone, overwriteIfExists, readContent } from '../utils';
...
function fixImport(options: PluginOptions): Rule {
  return async (tree: Tree): Promise<void> => {
    ...
    if (tree.exists(basicComponentPath)) {
      const content = readContent(tree, basicComponentPath).replace(`<div nz-menu style="width: 200px;">`, `<div nz-menu style="width: 200px;"><div nz-menu-item><header-rtl /></div>`);
...
export function pluginRTL(options: PluginOptions): Rule {
  return async (tree: Tree) => {
    ...
    return chain([mergeWith(apply(url('./files/rtl'), [move(`${project.sourceRoot}/app`), overwriteIfExists(tree)])), fixImport(options)]);
  };
}
