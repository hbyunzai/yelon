...
export function pluginIcon(options: PluginOptions): Rule {
  return (tree: Tree) => {
    ...
    console.log(`\n\n`);
    console.log(`生成成功，如果是首次运行，需要手动引用，参考：https://ng-yunzai.com/theme/icon/zh`);
    console.log(
      `Finished, if it's first run, you need manually reference it, refer to: https://ng-yunzai.com/theme/icon/en`
    );
    console.log(`\n\n`);
  };
}
