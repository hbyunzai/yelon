...
function removeDuplicateDependencies(): Rule {
  return (tree: Tree, context: SchematicContext) => {
    ...
    delete pkg.devDependencies['ng-yunzai-sts'];
    delete pkg.devDependencies['ng-yunzai-plugin-theme'];
    writePackage(tree, pkg);
    logStart(context, `Remove duplicate dev-dependencies: 'ng-yunzai-sts', 'ng-yunzai-plugin-theme'`);
  };
}
...
function finished(): Rule {
  return (_tree: Tree, context: SchematicContext) => {
    ...
    context.logger.info(
      colors.green(
        `  ✓ Congratulations, Abort more detail please refer to upgrade guide https://github.com/hbyunzai/ng-yunzai/issues/2347`
      )
    );
  };
}
