...
import { CacheService, YunzaiUser } from '@yelon/cache';
import { YunzaiConfigService, YunzaiSocketConfig, log, WINDOW } from '@yelon/util';
...
export class StompService implements OnDestroy {
  ...
  constructor(
    ...
    if (!this.user) {
      this.user = this.cacheService.get('_yz_user', { mode: 'none' });
    }
    if (!this.config) {
