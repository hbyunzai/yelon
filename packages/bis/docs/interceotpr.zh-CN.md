...
- 2.拦截器实现了对请求错误的统一处理
- 3.使用`YunzaiStartupService`后必须配对使用`YunzaiDefaultInterceptor`
...
const INTERCEPTOR_PROVIDES = [
  ...
  { provide: HTTP_INTERCEPTORS, useClass: YunzaiDefaultInterceptor, multi: true }
];
```

`app.module.ts`中拦截器统一配置于`INTERCEPTOR_PROVIDES`中，将`DefaultInterceptor`替换成`YunzaiDefaultInterceptor`即可
 ...
 */
@Injectable()
export class YunzaiDefaultInterceptor implements HttpInterceptor {
