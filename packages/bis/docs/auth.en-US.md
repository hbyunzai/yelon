...
`@yelon/bis` uses the built-in `YunzaiStartupService`, uses the method of interacting with `CAS`, and only provides the `front-end docking` mode (*management authorization/configuration change required*).
...
Since `YunzaiI18NService` is introduced in `YunzaiStartupService` to initialize `i18n`

```ts
const I18NSERVICE_PROVIDES = [{ provide: YUNZAI_I18N_TOKEN, useClass: YunzaiI18NService, multi: false }];
```

So you need to change the `I18NService` in `useClass` to `YunzaiI18NService`, **`i18n` configuration is located in `app.module.ts`**.
