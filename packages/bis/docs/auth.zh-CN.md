...
`@yelon/bis`使用内置`YunzaiStartupService`,采用与`CAS`交互的方式,仅提供`前台对接`模式(*需要管理端授权/更改配置*).
...
由于`YunzaiStartupService`中引入`YunzaiI18NService`来初始化`i18n`

```ts
const I18NSERVICE_PROVIDES = [{ provide: YUNZAI_I18N_TOKEN, useClass: YunzaiI18NService, multi: false }];
```

所以需要将`useClass`中的`I18NService`改变成`YunzaiI18NService`,**`i18n`配置位于`app.module.ts`**.
...
import { YunzaiAuthService } from './yz.auth.service';
import { YunzaiI18NService } from './yz.i18n.service';

// 递归将数据库中的菜单数据转换成 Ng-Yunzai支持的数据类型
export function mapYunzaiSideToYelonMenu(menus: Menu[]) {
  menus.forEach(menu => {
    ...
    menu.reuse = true;
    if (menu.children) {
      mapYunzaiSideToYelonMenu(menu.children);
    }
  });
}
